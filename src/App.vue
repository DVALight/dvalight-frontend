  ...
  <div :style="styleObject">
    ...
    <div class="content">
      ...
      <div class="controls">
        <div class="block">
          <p>color</p>
          <color-picker :disabled="isDisabledRCP" :h="h" @input="onColorInput" @select="onColorSelect" />
        </div>
        <div class="block modes">
          <p>timer</p>
          <button v-for="mode in MODES" :key="mode" class="mode" :class="{ active: mode === selectedMode }" :disabled="!isCheckedPwr" @click="selectMode(mode)">{{ mode }}</button>
        </div>
        <div class="block">
          <p>lights controls</p>
